...
void display_top(void)
{
	...
	if (graphic_card == HERCMONO) {
		...
		rectangle(frame[0].starty * 8, frame[0].startx * 16, frame[0].endy * 8, frame[0].endx * 16);
		floodfill(frame[0].starty * 8 + 1, frame[0].startx * 16 + 1, WHITE);
	} else {
		setfillstyle(SOLID_FILL, LIGHTGRAY);
		bar(frame[0].starty * 8, frame[0].startx * 16, frame[0].endy * 8, frame[0].endx * 16);
	}
...
void draw_border(int num)
{
	...
	if (num <= frame[0].count)
		xorbar(frame[num].starty * 8, (x - 2) * 16, (frame[num].starty + strlen(frame[0].menu[num - 1])) * 8,
		       (x - 1) * 16);

	if (graphic_card == HERCMONO) {
		setfillstyle(SOLID_FILL, BLACK);
		bar(frame[num].starty * 8, frame[num].startx * 16 + 8, frame[num].endy * 8, frame[num].endx * 16 - 8);
		rectangle(frame[num].starty * 8, frame[num].startx * 16 + 8, frame[num].endy * 8,
			  frame[num].endx * 16 - 8);
	} else {
		setfillstyle(SOLID_FILL, LIGHTGRAY);
		bar(frame[num].starty * 8, frame[num].startx * 16 + 8, frame[num].endy * 8, frame[num].endx * 16 - 8);
...
int get_resp(int num)
{
	...
	int arrow_choice, key_choice, j, x, y, ey;
	...
	if (num > 0) {
		xorbar(y * 8, (x + arrow_choice) * 16, ey * 8, (x + arrow_choice + 1) * 16);
		...
		xorbar(frame[arrow_choice + 1].starty * 8, (x - 1) * 16,
		       (frame[arrow_choice + 1].starty + strlen(frame[num].menu[arrow_choice])) * 8, x * 16);
...
void save_video(int num)
{
	...
	frame[num].p = malloc(imagesize(frame[num].starty * 8, frame[num].startx * 16,
					frame[num].endy * 8, frame[num].endx * 16));
	getimage(frame[num].starty * 8, frame[num].startx * 16,
		 frame[num].endy * 8, frame[num].endx * 16, frame[num].p);
}
...
void restore_video(int num)
{
	...
	if (num <= frame[0].count)
		xorbar(frame[num].starty * 8, (x - 2) * 16,
		       (frame[num].starty + strlen(frame[0].menu[num - 1])) * 8, (x - 1) * 16);

	putimage(frame[num].starty * 8, frame[num].startx * 16, frame[num].p, COPY_PUT);
